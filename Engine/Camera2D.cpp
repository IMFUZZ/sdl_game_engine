...
*/
	...
	void Engine::Camera2D::init(int a_screenWidth, int a_screenHeight) {
	...
	bool Engine::Camera2D::isBoxInView(const glm::vec2 & a_pos, const glm::vec2 & a_dims) {
		glm::vec2 
	...
	void Engine::Camera2D::setPosition(const glm::vec2 & a_pos) {
		_position = a_pos;
		_needsMatrixUpdate = true;
	}

	void Engine::Camera2D::translate(const glm::vec2 & a_translation) {
		setPosition(_position + a_translation);
	}

	void Engine::Camera2D::setScale(float a_scale) {
		_scale = a_scale;
		_needsMatrixUpdate = true;
	}

	void Engine::Camera2D::zoom(float a_zoomValue) {
		setScale(_scale + a_zoomValue);
	}


	glm::vec2 Engine::Camera2D::convertScreenToWorld(glm::vec2 screenCoords) {
		// set 0 as center of screen
		screenCoords -= glm::vec2(_screenWidth/2, _screenHeight/2);
		// Adjust to scale
		screenCoords /= _scale;
		// translate to the current world position
